...
start_server() {
    ...
    ssh -i ${root}container/key -o StrictHostKeychecking=no root@${WALLET_VM_ADDRESS}\
        "python3 Benchmarks/IPC/server.py ${port} &> /dev/null" &
    sleep 1
}
...
if [ "$1" == "VM" ]; then
	...
	sudo kill $VM
	sleep 5
	python parse.py "VM" > vm_result.txt
fi
...
if [ "$1" == "CVM" ]; then
	...
	make -C ${root} run &
        ...
        sudo kill $VM
        sleep 5
	python parse.py "CVM" > cvm_result.txt
fi
...
if [ "$1" == "kata" ]; then
	...
	kill $VM
	python parse.py "Kata Containers" > kata_result.txt
fi
