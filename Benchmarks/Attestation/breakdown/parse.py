...
def parse(filename):
    ...
    text = text.split("Attaching 4 probes...\n")[1].split("\n")
...
with open("result.csv", "w") as f:
    f.write("zygote,trustlet,input,output\n")
    for i in range(1,11):
        with open(f"trace_{i}","r") as f2:
            lines = f2.readlines()
        start = 0
        for l in range(len(lines)):
            if "Attaching" in lines[l]:
                start = l +1
                break
        lines = lines[2:]
        def ms(ns):
            return (ns / 1e6)
        zygote = ms(int(lines[7].split(": ")[0]) - int(lines[2].split(": ")[0]))
        trustlet = ms(int(lines[15].split(": ")[0]) - int(lines[14].split(": ")[0]))
        args = ms(int(lines[23].split(": ")[0]) - int(lines[22].split(": ")[0]))
        outs = ms(int(lines[27].split(": ")[0]) - int(lines[26].split(": ")[0]))
        f.write(f"{zygote},{trustlet},{args},{outs}\n")
