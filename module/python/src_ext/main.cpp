...
void load_file(const char* filename, uint8_t** buffer, uint64_t* buffer_size) {
    ...
    ::rewind(file);
    uint8_t* buf = (uint8_t*)::aligned_alloc(4096, size + 4096);
    for (uint64_t i = 0; i < size + 4096; i++) buf[i] = 0;
...
int create_trustlet(int fd, const int zygote_id) {
    struct monitor_call call;
...
int invoke_trustlet(int fd, const int trustlet_id) {
    struct monitor_call call;
